...
const LOG_TARGET: &str = "minotari::base_node::profile";
const PROFILE_PORT: u16 = 17100;
...
/// 启动Profile HTTP服务器
pub async fn start_profile_server() -> Result<(), Box<dyn std::error::Error>> {
    use axum::{
        routing::get,
        Router,
    };
    ...
    #[openapi(
        ...
        tags(
            (name = "Profile", description = "内存分析相关API")
        )
    )]
    struct ApiDoc;
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:{}", PROFILE_PORT)).await?;

    info!(target: LOG_TARGET, "Profile HTTP server listening at 0.0.0.0:{}", PROFILE_PORT);
